...
public class SaveViewDBFrame extends JFrame implements ActionListener, ItemListener{
	...
	private JComboBox<String> repdetailorderbyCb;
	private JComboBox<String> linorderbyCb;
	private JComboBox<String> lindetailorderbyCb;
	private final Dimension sizeOrderCb = new Dimension(200, 21);
	
	/**
	 * Constructor
	 * @param mf the AECFrame object
	 */
	public SaveViewDBFrame(AECFrame mf){
		this.mf=mf;
    	...
    	repdbagent = new DatabaseAgentSupport(radqcdbcon,"UNIQUE_ID", mf.aecRepeatabilityTable);
    	repdetaildbagent = new DatabaseAgentSupport(radqcdbcon,"ID", mf.aecRepeatabilityTableDetail);
    	lindbagent = new DatabaseAgentSupport(radqcdbcon,"UNIQUE_ID", mf.aecLinearityTable);
    	lindetaildbagent = new DatabaseAgentSupport(radqcdbcon,"ID", mf.aecLinearityTableDetail);
	...
	private void performQueryDB(){
		...
		try {
			...
			//Rep
			String s = "select * from " + mf.aecRepeatabilityTable+" where IDLINK = "+mf.IDLINK +" order by Unique_ID";
			...
			suportSpRep.add(scrollPane, BorderLayout.CENTER);//(aspRep, BorderLayout.CENTER);

			//JTable mainTable = aspRep.getTab();
			...
			mainTable.setFillsViewportHeight(true);
			suportSpRepDetail.add(scrollPane2, BorderLayout.CENTER);//aspRepDetail, BorderLayout.CENTER);
			
			//mainTable = aspRepDetail.getTab();
			nOutputRepDetail = mainTable.getRowCount();
			//-----------------------
			//Lin
			s = "select * from " + mf.aecLinearityTable+" where IDLINK = "+mf.IDLINK +" order by Unique_ID";
			...
			mainTable.setFillsViewportHeight(true);
			suportSpLin.add(scrollPane3, BorderLayout.CENTER);//aspLin, BorderLayout.CENTER);
			...
			suportSpLinDetail.add(scrollPane4, BorderLayout.CENTER);//aspLinDetail, BorderLayout.CENTER);
	...
	/**
	 * Update nested table (AEC repeatability)
	 */
	...
	//System.out.println("update kv detail fired");//nope
		...
		try {
			...
			String s = "select * from " + mf.aecRepeatabilityTable+ " where IDLINK = "+selID +" order by Unique_ID";

			//Connection con1 = DBConnection.getDerbyConnection(opens, "", "");
			//DBOperation.select(s, con1);
			//repdbagent.select(s);//NOT AGENT BEACUSE IT SELECT=>LOOP!!
			...
			//if (aspRepDetail != null){//not used these vars....				
	...
	/**
	 * Update nested table (AEC compensation)
	 */
	private void updateDetailTableLin() {
		...
		try {
			//=============populate some fields from main table
			String s = "select * from " + mf.aecLinearityTable+ " where IDLINK = "+selID +" order by Unique_ID";

			//Connection con1 = DBConnection.getDerbyConnection(opens, "", "");
			//DBOperation.select(s, con1);
			//lindbagent.select(s);//NOT AGENT BEACUSE IT SELECT=>LOOP!!
	...
	private JTabbedPane createMainPanel() {
		...
		repdetailorderbyCb = repdetaildbagent.getOrderByComboBox();
		repdetailorderbyCb.setMaximumRowCount(5);
		repdetailorderbyCb.setPreferredSize(sizeOrderCb);
		repdetailorderbyCb.addItemListener(this);
		JPanel dorderP = new JPanel();
		dorderP.setLayout(new FlowLayout(FlowLayout.CENTER, 20, 2));
		label = new JLabel(mf.resources.getString("sort.by"));//"Sort by: ");
		label.setForeground(RadQCFrame.foreColor);
		//dorderP.add(label);
		//dorderP.add(repdetailorderbyCb);
		...
		lindetailorderbyCb = lindetaildbagent.getOrderByComboBox();
		lindetailorderbyCb.setMaximumRowCount(5);
		lindetailorderbyCb.setPreferredSize(sizeOrderCb);
		lindetailorderbyCb.addItemListener(this);
		JPanel dorderP2 = new JPanel();
		dorderP2.setLayout(new FlowLayout(FlowLayout.CENTER, 20, 2));
		label = new JLabel(mf.resources.getString("sort.by"));//"Sort by: ");
		label.setForeground(RadQCFrame.foreColor);
		//dorderP2.add(label);
		//dorderP2.add(lindetailorderbyCb);
	...
	public void itemStateChanged(ItemEvent e) {
		...
		if (e.getSource() == reporderbyCb) {
			repsort();
		} else if (e.getSource() == repdetailorderbyCb) {
			repdetailsort();
		} else if (e.getSource() == linorderbyCb) {
			linsort();
		} else if (e.getSource() == lindetailorderbyCb) {
			lindetailsort();
		}
	}
	...
	@Deprecated
	private void repdetailsort() {
		//String orderbyS = (String) repdetailorderbyCb.getSelectedItem();
		//MULTIPLE LINKS NOT SUPPORTED
	}
	
	@Deprecated
	private void lindetailsort() {
		//String orderbyS = (String) repdetailorderbyCb.getSelectedItem();
		//MULTIPLE LINKS NOT SUPPORTED
	}
	
	/**
	 * Sorts data from compensation table
	 */
	...
	/**
	 * Save AEC repeatability QC test result
	 */
	private void saveRep(){
		
		if (!mf.isOktoSaveReapeatability){
		...
		try {			
			...
			psInsert = radqcdbcon.prepareStatement("insert into "//con1.prepareStatement("insert into "
					+ mf.aecRepeatabilityTable + " values " + "(?, ?, ?, ?, ?, ?)");
			...
			psInsert = radqcdbcon.prepareStatement("insert into " + mf.aecRepeatabilityTableDetail//con1.prepareStatement("insert into " + mf.aecRepeatabilityTableDetail
					+ " values " + "(?, ?, ?, ?)");
			for (int i = 0; i < n; i++) {
				int nrcrt = i + 1;
	...
	/**
	 * View all records from repeatability table
	 */
	...
	private void deleteRep(){
		try {
			...
			ResultSet res = s.executeQuery("SELECT * FROM " + mf.aecRepeatabilityTable);//deviceTable);
			...
			//now the detail
			ResultSet res1 = s.executeQuery("SELECT * FROM " + mf.aecRepeatabilityTableDetail);//deviceTable);
			while (res1.next()) {
	...
	/**
	 * Save AEC compensation QC test result
	 */
	private void saveLin(){
		if (!mf.isOktoSaveLinearity){
			JOptionPane.showMessageDialog(this,
		...
		try {			
			...
			psInsert = radqcdbcon.prepareStatement("insert into " + mf.aecLinearityTableDetail//con1.prepareStatement("insert into " + mf.aecLinearityTableDetail
					+ " values " + "(?, ?, ?, ?)");
			for (int i = 0; i < n; i++) {
				int nrcrt = i + 1;
				psInsert.setString(1, Convertor.intToString(nrcrt));
	...
	/**
	 * Select data from compensation table 
	 */
	private void selectTableLin(){
		...
		try {			
			...
			String s = "select * from " + mf.aecLinearityTable+" where IDLINK = "+mf.IDLINK +" order by Unique_ID";
	...
	/**
	 * View all records from compensation table
	 */
	...
	/**
	 * Delete an entry from compensation table 
	 */
	private void deleteLin(){
		try {
			...
			Statement s = radqcdbcon.createStatement(ResultSet.TYPE_FORWARD_ONLY,//con1.createStatement(ResultSet.TYPE_FORWARD_ONLY,
					ResultSet.CONCUR_UPDATABLE);
			ResultSet res = s.executeQuery("SELECT * FROM " + mf.aecLinearityTable);//deviceTable);
			//PreparedStatement psUpdate = null;
			while (res.next()) {
				int id = res.getInt("Unique_ID");//("ID");
				int id2 = res.getInt("IDLINK");//("ID");
