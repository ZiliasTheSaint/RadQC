...
/**
 * Computes total filtration based on known attenuators. Since the X-ray tube filtration 
 * is taken from database (previously computed by HVL test), this class is not used here.  
...
public class TubeAttenuators extends JFrame implements ActionListener{
	 ...
	 */
	public TubeAttenuators(DosimetryFrame mf){
	...
	private JPanel createMainPanel()
	{
		...
		pathP.add(matTf);
		pathP.add(loadB);
		pathP.setBackground(RadQCFrame.bkgColor);
		...
		d1.add(lbl);
		d1.add(gmatTf);
		d1.setBackground(RadQCFrame.bkgColor);

		JPanel d2=new JPanel();
		d2.setLayout(new FlowLayout(FlowLayout.CENTER,10,0));
		d2.add(addB);
		d2.add(deleteB);
		d2.setBackground(RadQCFrame.bkgColor);
		...
		expDataAllP.add(d1);
		expDataAllP.add(d2);
		expDataAllP.setBackground(RadQCFrame.bkgColor);
		...
		d3.add(expDataAllP);
		d3.add(listP);
		d3.setBackground(RadQCFrame.bkgColor);
		...
		d4.add(calcB);
		d4.add(spectruxB);
		d4.setBackground(RadQCFrame.bkgColor);
