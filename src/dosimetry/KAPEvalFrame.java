...
/**
 * Evaluation of Kerma-Area-Product (KAP) for radiography.
 * @author Dan Fulea, 04 May 2015
 */
@SuppressWarnings("serial")
public class KAPEvalFrame extends JFrame implements ActionListener{
	...
	/**
	 * Constructor
	 * @param mf the DosimetryFrame object
	 */
	public KAPEvalFrame(DosimetryFrame mf){
		this.mf=mf;
	...
	private JPanel createMainPanel() {
		...
		focusToTableTf.setText("100");
		focusToPatientTf.setText("100");
		fieldXTf.setText("39");
		fieldYTf.setText("39");
		bsfTf.setText("1.3");
...
//System.out.println("doseTabledet = "+doseAtTableLevel+"; doseattablelvlPHANTOM= "+doseAtTableLevelPhantom+" distanceTableDet= "+focusToTable+"distanceTablePHANTOM= "+focusToTablePhantom);		
		//====================================
		//double KAP=doseAtTableLevel*fieldX*fieldY;//mGy*cm2
		...
		textArea.append(mf.resources.getString("kap.KAP.ugymm2")+
				Convertor.formatNumber(KAP,2)+"\n");
		//====================
		String ageGroup=mf.getAgeGroup();
		double projection=mf.getProjectionAngle();
		double scaleXY=mf.getScaleXY();
		double scaleZ=mf.getScaleZ();
		double[] fcafsd=mf.getFCAandFSDfromSID(focusToTablePhantom, ageGroup, projection,scaleXY);//patient!
		
		textArea.append(mf.resources.getString("phantom.scaleXY")+
				Convertor.formatNumber(scaleXY,2)+mf.resources.getString("phantom.scaleZ")+
				Convertor.formatNumber(scaleZ,2)+"\n");
		
		textArea.append(mf.resources.getString("FCA.cm")+
				Convertor.formatNumber(fcafsd[0],2)+"\n");
		textArea.append(mf.resources.getString("FSD.cm")+
				Convertor.formatNumber(fcafsd[1],2)+"\n");
		
		double doseAtPatientEntrance=doseAtTableLevelPhantom*focusToTablePhantom*
		focusToTablePhantom/fcafsd[1]/fcafsd[1];//mGy
//System.out.println("dosePatient = "+doseAtPatientEntrance);		
		...
		mf.focusmidplaneTf.setText(Convertor.doubleToString(fcafsd[0]));
	}
	...
	private boolean updateFields(){
		...
		//if (mf.EXAMINATION_ID==0){//RADIOGRAPHY
		double fieldX_atFCA=mf.getRawFieldX();
		double fieldY_atFCA=mf.getRawFieldY();
		double scaleZ=mf.getScaleZ();
		double scaleXY=mf.getScaleXY();
		
		fieldX_atFCA=fieldX_atFCA*scaleXY;
		fieldY_atFCA=fieldY_atFCA*scaleZ;
		
		//System.out.println("X "+fieldX_atFCA+"; Y "+fieldY_atFCA);
		String ageGroup=mf.getAgeGroup();
		double projection=mf.getProjectionAngle();
		double[] fcafsd=mf.getFCAandFSDfromSID(focusToTablePhantom, ageGroup, projection,scaleXY);//patient!
		//FCA=fcafsd[0]
		
		double fieldX_table_patientGeometry =  fieldX_atFCA*focusToTablePhantom/fcafsd[0];//Thales
		double fieldY_table_patientGeometry =  fieldY_atFCA*focusToTablePhantom/fcafsd[0];//Thales
