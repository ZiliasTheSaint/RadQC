...
/**
 * Class for saving medical unit.
 * 
 * @author Dan Fulea, 22 Apr. 2015
 */
@SuppressWarnings("serial")
public class MUFrame extends JFrame implements ActionListener, ItemListener{
	...
	private int IDMU = 0;
	private JLabel idmulabel=null;
	
	private JTextField muTf = new JTextField(25);
	 ...
	 */
	public MUFrame(RadQCFrame mf){
		resources = ResourceBundle.getBundle(BASE_RESOURCE_CLASS);
		//DBConnection.startDerby();//just in case is closed
		
		this.setTitle(resources.getString("MedicalUnitFrame.NAME"));		
	...
	private JPanel createMainPanel() {
		...
		label.setForeground(RadQCFrame.foreColor);
		p1P.add(label);
		p1P.add(muTf);muTf.addActionListener(this);
		...
		label.setForeground(RadQCFrame.foreColor);
		p4P.add(label);
		String str=Convertor.intToString(IDMU);
		idmulabel = new JLabel(str);
		idmulabel.setForeground(RadQCFrame.foreColor);
		p4P.add(idmulabel);
	...
	private void performQueryDb() {
		try {
			...
			IDMU = mainTable.getRowCount();// last ID
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (command.equals(INSERT_COMMAND)) {
			...
			delete();
		} else if (arg0.getSource() == muTf) {// press enter!
			insert();
		}
	}
	...
	private void updateControls(){
		...
		try {
			...
			rs = st.executeQuery("SELECT * FROM " + muTable);
			...
			for(int i=0;i<muV.size();i++){
				...
				mf.muCb.addItem(muV.elementAt(i));
			}
			//=====================================
			//con1.commit();
	...
	private void delete(){
		try {
			...
			ResultSet res = s.executeQuery("SELECT * FROM " + muTable);
			...
			while (res.next()) {
				...
				if (id == selID) {
					...
					// make
					// on-the fly update
					psUpdate = radqcdbcon.prepareStatement("update " + muTable//con1.prepareStatement("update " + muTable
							+ " set ID=? where ID=?");
