...
/**
 * Class for saving XRay device types.
 * 
 * @author Dan Fulea, 22 Apr. 2015
 */
@SuppressWarnings("serial")
public class DeviceTypeFrame extends JFrame implements ActionListener, ItemListener{
	...
	private int IDDEVICETYPE = 0;
	private JLabel iddevicetypelabel=null;
	
	private JTextField deviceTypeTf = new JTextField(25);
	 ...
	 */
	public DeviceTypeFrame(RadQCFrame mf){
		resources = ResourceBundle.getBundle(BASE_RESOURCE_CLASS);
		//DBConnection.startDerby();//just in case is closed
		
		this.setTitle(resources.getString("DeviceTypeFrame.NAME"));		
	...
	private JPanel createMainPanel() {
		...
		label.setForeground(RadQCFrame.foreColor);
		p1P.add(label);
		p1P.add(deviceTypeTf);deviceTypeTf.addActionListener(this);
		...
		label.setForeground(RadQCFrame.foreColor);
		p4P.add(label);
		String str=Convertor.intToString(IDDEVICETYPE);
		iddevicetypelabel = new JLabel(str);
		iddevicetypelabel.setForeground(RadQCFrame.foreColor);
		p4P.add(iddevicetypelabel);
	...
	private void performQueryDb() {
		try {
			...
			//String s = "select * from " + deviceTypeTable;
			...
			IDDEVICETYPE = mainTable.getRowCount();// last ID
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (command.equals(INSERT_COMMAND)) {
			...
			delete();
		} else if (arg0.getSource() == deviceTypeTf) {// press enter!
			insert();
		}
	}
	...
	private void insert(){
		...
		String muStr=deviceTypeTf.getText();
		...
		try {
			...
			IDDEVICETYPE=IDDEVICETYPE+1;
			updateControls();
			
			deviceTypeTf.setText("");
			deviceTypeTf.requestFocusInWindow();
	...
	private void updateControls(){
		...
		try {
			...
			rs = st.executeQuery("SELECT * FROM " + deviceTypeTable);
			...
			for(int i=0;i<muV.size();i++){
				...
				mf.deviceCb.addItem(muV.elementAt(i));
			}
			//=====================================
			//con1.commit();
	...
	private void delete(){
		try {
			...
			ResultSet res = s.executeQuery("SELECT * FROM " + deviceTypeTable);
			...
			while (res.next()) {
				...
				if (id == selID) {
					...
					// make
					// on-the fly update..few records, we can do this operation!
					psUpdate = radqcdbcon.prepareStatement("update " + deviceTypeTable//con1.prepareStatement("update " + deviceTypeTable
							+ " set ID=? where ID=?");
